...
function newsadds(){
	...
	$unsetname = $uname;
	$zapytanie = 'SELECT * FROM '.$unsetname.' WHERE dateremove > NOW() ORDER BY dateremove DESC LIMIT 10';
	$pytanko = ' WHERE dateremove != 0';
	try{					
			...
			if($result = $connect->query($pytanietymczasowa)){
				...
				if($result = $connect->query('SELECT * FROM '.$unsetname)){
					...
					if($wyniki>0){
						...
						echo '<div>Dodaj swoje nowe ogłoszenie i zobacz je tutaj!</div>';
					}
							...
							for($r = 1; $r <= $wyniki; $r++){
								...
								echo '
									<div class="lh-content">
									  ...
									  <h3><a title="'.$w['Tytul'].'" href="'.$w['Odnosnik'].'?id='.$w['ID'].'">'.$Tytul.'</a></h3>
									  <span style="display: inline-block;" class="icon-tags"></span> <address style="display: inline-block;">'.$Cena;
									  if($w['Negocjacja'] == 'on'){
										echo ' do negocjacji';
									  }
									  echo '</address>
