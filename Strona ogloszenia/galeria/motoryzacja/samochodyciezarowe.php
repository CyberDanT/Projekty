	...
	<body>
		...
		<div id="menu">
			<div id="nav">
				...
				<center>
					<div id="search">
						<div id="search1">
							<span style="font-size: 40px; color: white; text-shadow: 1px 1px black;">Samochody ciężarowe</span>
						</div>
					</div>
				</center>
				...
				<div style="margin-left: auto; margin-right: auto;">
					<form id="myForm" method="get"> 
						<div class="inputsearchblock">
							<div>
								...
								<div><select id="Marka" class="inputsearch" name="Marka"> 
									...
									<option <?php  if(@$_GET['Marka'] == 'DAF'){ echo 'selected'; } ?> class="searchopt" value="DAF">DAF</option>
									<option <?php  if(@$_GET['Marka'] == 'Iveco'){ echo 'selected'; } ?> class="searchopt" value="Iveco">Iveco</option>
									<option <?php  if(@$_GET['Marka'] == 'Jelcz'){ echo 'selected'; } ?> class="searchopt" value="Jelcz">Jelcz</option>								
									<option <?php  if(@$_GET['Marka'] == 'MAN'){ echo 'selected'; } ?> class="searchopt" value="MAN">MAN</option>
									...
									<option <?php  if(@$_GET['Marka'] == 'Scania'){ echo 'selected'; } ?> class="searchopt" value="Scania">Scania</option>
									<option <?php  if(@$_GET['Marka'] == 'Star'){ echo 'selected'; } ?> class="searchopt" value="Star">Star</option>
							...
							<div>
								...
								<datalist id="moc_silnika">
									<select id="ms">
										...
										<option class="searchopt" value="300" label="300 KM">
										<option class="searchopt" value="500" label="500 KM">
										<option class="searchopt" value="600" label="600 KM">
										<option class="searchopt" value="700" label="700 KM">
										<option class="searchopt" value="800" label="800 KM">
									</select>
								</datalist>
							</div>
						</div>
							...
							<div>
								...
								<datalist id="poj_silnika">
									<select id="ps">
										...
										<option class="searchopt" value="15000" label="15 000 cm³">
									</select>
								</datalist>
							</div>
						</div>
							...
							<div>
								...
								<datalist id="przebieg">
									<select id="prs">
										...
										<option class="searchopt" value="400000" label="400 000 km">
									</select>
								</datalist>
							</div>
						</div>
						<div style="float: right;">
							...
							<a class="submitsearch" onclick="window.location.href='samochodyciezarowe.php?page=<?php if(isset($_GET['page'])){echo $_GET['page'];}else{echo '0';}; ?>'">Usuń filtry</a>
						</div>
					</form>
				</div>
		...
		// --------------------------------------------- WYŚWIETLENIE WYNIKÓW ------------------------------------------------------------------
								...
								if(!$connect->set_charset("utf8")){
									...
									if($result = $connect->query($zapytanie)){
										...
										if($wyniki>0){
											...
											for($r = 1; $r <= $wyniki; $r++){
												$w = $result->fetch_assoc();
												echo '<div onclick="window.location = (\'ogloszenia-samochody-ciezarowe.php?id='.$w['ID'].'\');" class="oglcon1" style="background: #fafad2">';
									...
									if($result = @$connect->query($zapytanie1)){
										...
										if($wyniki>0){
											...
											for($r = 1; $r <= $wyniki; $r++){
												...
												$date = date('Y-m-d H:i:s');
												//echo $date;
												echo '<div onclick="window.location = (\'ogloszenia-samochody-ciezarowe.php?id='.$w['ID'].'\');" class="oglcon1">';
												echo '<div class="oglimgg">';
												for($i=1; $i<=8; $i++){
													if($w['Photo'.$i] != ''){
														$file = '../galeria/aktywne/'.$w['Photo'.$i];
						...
						echo '<div style="display: inline-flex; margin-top: 15px;">';	
							...
							echo '
							<a href="samochodyciezarowe.php?page=0'.$link.'&submitsearch=Szukaj">
							...
							<a href="samochodyciezarowe.php?page='.$pagep.''.$link.'&submitsearch=Szukaj">
								...
								for($p = $page+1; $p<=$page+9; $p++){
									$echo = $p - 1;
									if($echo == $page){
										echo '<a href="samochodyciezarowe.php?page='.$echo.''.$link.'&submitsearch=Szukaj"><div class="pagesquare" style="margin-top: 15px;">'.$p.'</div></a>';
									}else{
										if($echo < $lStron-1){
											echo '<a href="samochodyciezarowe.php?page='.$echo.''.$link.'&submitsearch=Szukaj"><div class="pagesquare">'.$p.'</div></a>';
										}
									}
								}
								
							echo '<a href="samochodyciezarowe.php?page='.$pagen.''.$link.'&submitsearch=Szukaj">	
									<div style="cursor: pointer; margin: 5px; font-size: 25px; border: 1px solid darkred; border-radius: 15px; padding: 10px; background: white; width: 50px;">
									>
								</div>
							</a>';
