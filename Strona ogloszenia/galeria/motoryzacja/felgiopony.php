	...
	<body>
		...
		<div id="menu">
			<div id="nav">
				...
				<center>
					<div id="search">
						<div id="search1">
							<span style="font-size: 40px; color: white; text-shadow: 1px 1px black;">Felgi i opony</span>
						</div>
					</div>
				</center>
				...
				<div style="margin-left: auto; margin-right: auto;">
					<form id="myForm" method="get"> 	
						...
						<div style="float: right;">
							...
							<a class="submitsearch" onclick="window.location.href='felgiopony.php?page=<?php if(isset($_GET['page'])){echo $_GET['page'];}else{echo '0';}; ?>'">Usuń filtry</a>
						</div>
					</form>
				</div>
		...
		// --------------------------------------------- WYŚWIETLENIE WYNIKÓW ------------------------------------------------------------------
								...
								if(!$connect->set_charset("utf8")){
									...
									if($result = $connect->query($zapytanie)){
										...
										if($wyniki>0){
											...
											for($r = 1; $r <= $wyniki; $r++){
												$w = $result->fetch_assoc();
												echo '<div onclick="window.location = (\'ogloszenia-felgi-opony.php?id='.$w['ID'].'\');" class="oglcon1" style="background: #fafad2">';
												...
												echo 'Motoryzacja ● Felgi i opony';
												echo '</span></div>';
									...
									if($result = @$connect->query($zapytanie1)){
										...
										if($wyniki>0){
											...
											for($r = 1; $r <= $wyniki; $r++){
												...
												$date = date('Y-m-d H:i:s');
												//echo $date;
												echo '<div onclick="window.location = (\'ogloszenia-felgi-opony.php?id='.$w['ID'].'\');" class="oglcon1">';
												echo '<div class="oglimgg">';
												for($i=1; $i<=3; $i++){
													if($w['Photo'.$i] != ''){
														$file = '../galeria/aktywne/'.$w['Photo'.$i];
						...
						echo '<div style="display: inline-flex; margin-top: 15px;">';	
							...
							echo '
							<a href="felgiopony.php?page=0'.$link.'&submitsearch=Szukaj">
							...
							<a href="felgiopony.php?page='.$pagep.''.$link.'&submitsearch=Szukaj">
								...
								for($p = $page+1; $p<=$page+9; $p++){
									$echo = $p - 1;
									if($echo == $page){
										echo '<a href="felgiopony.php?page='.$echo.''.$link.'&submitsearch=Szukaj"><div class="pagesquare" style="margin-top: 15px;">'.$p.'</div></a>';
									}else{
										if($echo < $lStron-1){
											echo '<a href="felgiopony.php?page='.$echo.''.$link.'&submitsearch=Szukaj"><div class="pagesquare">'.$p.'</div></a>';
										}
									}
								}
								
							echo '<a href="felgiopony.php?page='.$pagen.''.$link.'&submitsearch=Szukaj">	
									<div style="cursor: pointer; margin: 5px; font-size: 25px; border: 1px solid darkred; border-radius: 15px; padding: 10px; background: white; width: 50px;">
									>
								</div>
							</a>';
