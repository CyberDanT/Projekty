...
<html lang="pl">
  <head>
    <title>Wszystko w elektronika - BezGrosika.pl serwis ogłoszeniowy</title>
	<link rel="shortcut icon" href="img/grosiky.png">
	<meta name="keywords" content="serwis ogłoszeniowy, elektronika komputery stacjonarne w, laptopy, telefony, tablety, akcesoria do komputerów, lampki, żarówki i ledy, myszki komputerowe, bezgrosika, ogłoszenia elektronika komputery"/>
	<meta name="description" content="Wszystko w kategorii elektronika, komputery i laptopy, telefony, tablety, akcesoria komputerowe, myszki komputerowe, klawiatury, głośniki, telewizory full HD i inne w ogłoszeniach BezGrosika.pl"/>
	<meta charset="utf-8">
          ...
          <div class="col-md-10" data-aos="fade-up" data-aos-delay="400">
            ...
            <div class="row justify-content-center mt-5">
              <div class="col-md-8 text-center">
                <h1>Elektronika</h1>
                <p class="mb-0">Wszystkie ogłoszenia w elektronice</p>
              </div>
            </div>
      ...
      <div class="container">
        <div class="row mb-5">
          <div class="col-md-7 text-left border-primary">
            <h2 class="font-weight-light text-primary">Wszystkie w elektronice</h2>
          </div>
        </div>
        <div class="row mt-5">
		  ...
		  <div class="col-lg-6">
            <div class="d-block d-md-flex listing">
              ...
              <div class="lh-content">
                ...
                <h3><a href="#">Sprzedam telefon</a></h3>
                <address style="display: inline-block;"><span style="display: inline-block;" class="icon-tags"></span> 520 PLN</address>
              </div>
            </div>
		  </div>
					...
					try{					
						...
						if(!$connect->set_charset("utf8")){
							...
							if($result = $connect->query($pytanietymczasowa)){
								...
								$pytanko1 = 'SELECT * FROM elektronika_komputery';
								...
								$connect->select_db($administratorbazy.'elektronika');
								$pytanko1 = 'SELECT * FROM elektronika_telewizory';
								if($result = $connect->query($pytanko1)){
									//echo 'wybrano samochody_osobowe';
									$wyniki = $result->num_rows;
									if($wyniki>0){
								...
								$connect->select_db($administratorbazy.'elektronika');
								$pytanko1 = 'SELECT * FROM elektronika_telefony';
								if($result = $connect->query($pytanko1)){
									//echo 'wybrano samochody_osobowe';
									$wyniki = $result->num_rows;
									if($wyniki>0){
								...
								$connect->select_db($administratorbazy.'elektronika');
								$pytanko1 = 'SELECT * FROM elektronika_tablety';
								if($result = $connect->query($pytanko1)){
									//echo 'wybrano samochody_osobowe';
									$wyniki = $result->num_rows;
									if($wyniki>0){
								...
								$connect->select_db($administratorbazy.'elektronika');
								$pytanko1 = 'SELECT * FROM elektronika_konsole';
								if($result = $connect->query($pytanko1)){
									//echo 'wybrano samochody_osobowe';
									$wyniki = $result->num_rows;
									if($wyniki>0){
								...
								$connect->select_db($administratorbazy.'elektronika');
								$pytanko1 = 'SELECT * FROM elektronika_akcesoria';
								if($result = $connect->query($pytanko1)){
									//echo 'wybrano samochody_osobowe';
									$wyniki = $result->num_rows;
									if($wyniki>0){
								...
								$connect->select_db($administratorbazy.'elektronika');
								$pytanko1 = 'SELECT * FROM elektronika_pozostale';
								if($result = $connect->query($pytanko1)){
									//echo 'wybrano samochody_osobowe';
									$wyniki = $result->num_rows;
									if($wyniki>0){
								...
								if($result = @$connect->query($pytanie)){
									...
									<div class="col-12 mt-5 text-center">
									  <div class="custom-pagination">';
										if($lStron != 0){
											if($showpage != 1){
												echo '<a href="elektronika.php?page='.$previouspage.'">'.$seepreviouspage.'</a>';
											}else{
												echo '<a href="#">_</a>';
											}
											...
											if($lStron == $showpage){
												...
												echo '<a href="elektronika.php?page='.$nextpage.'">'.$seepage.'</a>';
											}
