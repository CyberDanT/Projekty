...
<head>
  ...
  <div class="site-wrap">
	...
	$_SESSION['info_ogl2'] = 'samochody osobowe';
	require_once("../php/connect.php");
	$db_name = $administratorbazy."motoryzacja";
	$zapytanie = 'SELECT * FROM samochody_osobowe WHERE ID="'.$ID.'"';
								  ...
								  <div class="col-lg-8">
									...
									<div class="mb-4">
									  <div class="slide-one-item home-slider owl-carousel">';
										$num = 8; // Maksymalna ilość zdjęć w danej kategorii
										$stop = false;
										$number = 1;
										for($p = 1; $p<=$num; $p++){
										...
										<center>
											<div class="ogl-icons-div">
												<form method="post" target = "_blank" action="../promotion.php">
													<input type="hidden" name="category" value="samochody_osobowe">
									...
									echo '<div><b>Marka</b></div> <div>'.$w['Marka'].'</div><br>';
									echo '<div><b>Model</b></div> <div>'.$w['Model'].'</div><br>';
									echo '<div><b>Moc silnika</b></div> <div>'.$w['Mocsilnika'].' KM</div><br>';
									$Pojemnosc = number_format($w['Pojsilnika'],0," "," ");
									echo '<div><b>Poj. silnika</b></div> <div>'.$Pojemnosc.' cm³</div><br>';
									echo '<div><b>Rok produkcji</b></div> <div>'.$w['Rokprodukcji'].'</div><br>';
									$Przebieg = number_format($w['Przebieg'],0," "," ");
									echo '<div><b>Przebieg</b></div> <div>'.$Przebieg.' km</div><br>';
									echo '</div>';
									// Druga
									echo '<div style="display: inline-block;">';
									echo '<div><b>Nadwozie</b></div> <div>'.$w['Nadwozie'].'</div><br>';
									echo '<div><b>Paliwo</b></div> <div>'.$w['Paliwo'].'</div><br>';
									echo '<div><b>Skrzynia biegów</b></div> <div>'.$w['Skrzynia'].'</div><br>';
									echo '<div><b>Kolor</b></div> <div>'.$w['Kolor'].'</div><br>';
									echo '<div><b>Stan techniczny</b></div> <div>'.$w['Stantechniczny'].'</div><br>';
									echo '<div><b>Stan użytkowy</b></div> <div>';
