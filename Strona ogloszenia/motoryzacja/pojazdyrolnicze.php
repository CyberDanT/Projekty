...
<html lang="pl">
  <head>
	<title>Ogłoszenia BezGrosika.PL - pojazdy rolnicze</title>
	<meta name="description" content="Motoryzacja, pojazdy rolnicze, nowe i używane ciągniki, przyczepy, sprzęty rolnicze, traktory..."/>
	<meta name="keywords" content="ogłoszenia, BezGrosika, .pl, sprzedam, bez grosika, kupie, traktor, ciągnik, rolnicze, przyczepa"/>
          ...
          <div class="col-md-10" data-aos="fade-up" data-aos-delay="400">
            ...
            <div class="row justify-content-center mt-5">
              <div class="col-md-8 text-center">
                <h1>Pojazdy rolnicze</h1>
                <p class="mb-0">Wszystkie ogłoszenia w sprzętach rolniczych</p>
              </div>
            </div>
		...
		<form id="myForm" method="get"> 
			<input <?php
				...
				<div>
					...
					<span class="filters-row-3"><a class="filters-input-a" onclick="window.location.href='pojazdyrolnicze.php?page=<?php if(isset($_GET['page'])){echo $_GET['page'];}else{echo '0';}; ?>'">Usuń filtry</a></span>
				</div>
		</form>
      ...
      <div class="container">
        <div class="row mb-5">
          <div class="col-md-7 text-left border-primary">
            <h2 class="font-weight-light text-primary">Wszystkie w pojazdach i sprzętach rolniczych</h2>
          </div>
        </div>
        <div class="row mt-5">
		  ...
		  <div class="col-lg-6">
            <div class="d-block d-md-flex listing">
              ...
              <div class="lh-content">
                ...
                <h3><a href="#">Traktor na sprzedaż</a></h3>
		...
		<?php
							$zapytanie = 'SELECT * FROM pojazdy_rolnicze WHERE datepromotion > NOW()';
							$zapytanie1 = 'SELECT * FROM pojazdy_rolnicze WHERE datepromotion < NOW()';
							...
							try {
								...
								if(!$connect->set_charset("utf8")){
									...
									if($result = $connect->query('SELECT * FROM pojazdy_rolnicze')){
										...
										if($wyniki>0){
											for($r = 1; $r <= $wyniki; $r++){
												...
												if($w['dateremove'] <= $date){
													...
													@$connect->query('DELETE FROM pojazdy_rolnicze WHERE ID='.$w['ID'].'');
												}
											}
										}
										$result->free_result();
		...
		// --------------------------------------------- WYŚWIETLENIE WYNIKÓW ------------------------------------------------------------------
									...
									if($result = $connect->query($zapytanie)){
										...
										if($wyniki>0){
											...
											for($r = 1; $r <= $wyniki; $r++){
												...
												for($i=1; $i<=8; $i++){
													if($w['Photo'.$i] != ''){
														...
														if(!isset($glowne)){
															if(@file_exists($file)){
																$glowne = $w['Photo'.$i];
																echo '<a href="ogloszenia-pojazdy-rolnicze.php?id='.$w['ID'].'" class="img d-block" style="background-image: url(\'../galeria/aktywne/'.$w['Photo'.$i].'\')"></a>';
															}
														}
													}
												}
												...
												if(!isset($glowne)){
													echo '<a href="ogloszenia-pojazdy-rolnicze.php?id='.$w['ID'].'" class="img d-block" style="background-image: url(\'../img/camera.png\')"></a>';
												}
												echo '
												<div class="lh-content">
														<span class="category" style="background-color: #f89d13;">Motoryzacja</span>
														<a href="ogloszenia-pojazdy-rolnicze.php?id='.$w['ID'].'" class="bookmark"><span class="icon-star text-warning" style="font-size: 25px;"></span></a>
														<h3><a href="ogloszenia-pojazdy-rolnicze.php?id='.$w['ID'].'" title="'.$w['Tytul'].'">'.$Tytul.'</a></h3>
						...
						try{
							...
							if (!$connect->set_charset("utf8")) {
								...
								$pytanie = 'SELECT * FROM pojazdy_rolnicze';
							...
							<div class="col-12 mt-5 text-center">
							  <div class="custom-pagination">';
								if($lStron != 0){
									if($showpage != 1){
										echo '<a href="pojazdyrolnicze.php?page='.$previouspage.$link.'&submitsearch=Szukaj">'.$seepreviouspage.'</a>';
									}else{
										echo '<a href="#">_</a>';
									}
									...
									if($lStron == $showpage){
										...
										echo '<a href="pojazdyrolnicze.php?page='.$nextpage.$link.'&submitsearch=Szukaj">'.$seepage.'</a>';
									}
