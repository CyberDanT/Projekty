...
<html lang="pl">
	<head>
		<meta charset="utf-8"/>
		<title>Ogłoszenia BezGrosika.PL - elektronika, komputery</title>
		<meta name="description" content="Elektronika, komputery, kupię, sprzedam komputery, pamięć RAM, procesory, dyski SSD, dyski HDD, karty graficzne..."/>
		<meta name="keywords" content="ogłoszenia, BezGrosika, .pl, sprzedam, kupie, komputery, sprzedam karte graficzną GTX, MSI, Nvidia, Radeon, elektronika"/>
		...
		<div id="menu">
			<div id="nav">
				...
				<center>
					<div id="search">
						<div id="search1">
							<span style="font-size: 40px; color: white; text-shadow: 1px 1px black;">Komputery</span>
						</div>
					</div>
				</center>
					...
					<form id="myForm" method="get"> 	
						<div class="inputsearchblock">
							<div>
								...
								<datalist id="cena">
									<select id="cs">
										<option class="searchopt" value="100" label="100 zł">
										<option class="searchopt" value="300" label="300 zł">
										<option class="searchopt" value="500" label="500 zł">
										<option class="searchopt" value="1500" label="1 500 zł">
										<option class="searchopt" value="3500" label="3 500 zł">
									</select>
								</datalist>
							</div>
						</div>
						<div style="float: right;">
							...
							<a class="submitsearch" onclick="window.location.href='komputery.php?page=<?php if(isset($_GET['page'])){echo $_GET['page'];}else{echo '0';}; ?>'">Usuń filtry</a>
						</div>
					</form>
				</div>
				
				<?php
							$zapytanie = 'SELECT * FROM elektronika_komputery WHERE datepromotion > NOW()';
							$zapytanie1 = 'SELECT * FROM elektronika_komputery WHERE datepromotion < NOW()';
							...
							try {
								...
								if(!$connect->set_charset("utf8")){
									...
									if($result = $connect->query('SELECT * FROM elektronika_komputery')){
										...
										if($wyniki>0){
											for($r = 1; $r <= $wyniki; $r++){
												...
												if($w['dateremove'] <= $date){
													...
													@$connect->query('DELETE FROM elektronika_komputery WHERE ID='.$w['ID'].'');
												}
											}
										}
										$result->free_result();
		...
		// --------------------------------------------- WYŚWIETLENIE WYNIKÓW ------------------------------------------------------------------
								...
								if(!$connect->set_charset("utf8")){
									...
									if($result = $connect->query($zapytanie)){
										...
										if($wyniki>0){
											...
											for($r = 1; $r <= $wyniki; $r++){
												$w = $result->fetch_assoc();
												echo '<div onclick="window.location = (\'ogloszenia-komputery.php?id='.$w['ID'].'\');" class="oglcon1" style="background: #fafad2">';
												...
												echo '<img src="../img/computericon1.png" style="margin-left: 15px; width: 30px; float: left;">';
												
												echo '<span style="margin-left: 5px; line-height: 30px;">';
												echo 'Elektronika ● Komputery';
												echo '</span></div>';
									...
									if($result = @$connect->query($zapytanie1)){
										...
										if($wyniki>0){
											...
											for($r = 1; $r <= $wyniki; $r++){
												...
												$date = date('Y-m-d H:i:s');
												//echo $date;
												echo '<div onclick="window.location = (\'ogloszenia-komputery.php?id='.$w['ID'].'\');" class="oglcon1">';
												echo '<div class="oglimgg">';
												for($i=1; $i<=3; $i++){
													if($w['Photo'.$i] != ''){
														$file = '../galeria/aktywne/'.$w['Photo'.$i];
						...
						try{
							...
							if (!$connect->set_charset("utf8")) {
								...
								$pytanie = 'SELECT * FROM elektronika_komputery';
						...
						echo '<div style="display: inline-flex; margin-top: 15px;">';	
							...
							echo '
							<a href="komputery.php?page=0'.$link.'&submitsearch=Szukaj">
							...
							<a href="komputery.php?page='.$pagep.''.$link.'&submitsearch=Szukaj">
								...
								for($p = $page+1; $p<=$page+9; $p++){
									$echo = $p - 1;
									if($echo == $page){
										echo '<a href="komputery.php?page='.$echo.''.$link.'&submitsearch=Szukaj"><div class="pagesquare" style="margin-top: 15px;">'.$p.'</div></a>';
									}else{
										if($echo < $lStron-1){
											echo '<a href="komputery.php?page='.$echo.''.$link.'&submitsearch=Szukaj"><div class="pagesquare">'.$p.'</div></a>';
										}
									}
								}
								
							echo '<a href="komputery.php?page='.$pagen.''.$link.'&submitsearch=Szukaj">	
									<div style="cursor: pointer; margin: 5px; font-size: 25px; border: 1px solid darkred; border-radius: 15px; padding: 10px; background: white; width: 50px;">
									>
								</div>
							</a>';
